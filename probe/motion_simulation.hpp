...
void calculate_position(Robot& robot){
    double delta_x = robot.motion_vector.linear_velocity * cos(robot.angle * M_PI / 180) * robot.discrete_time_interval;
    double delta_y = robot.motion_vector.linear_velocity * sin(robot.angle * M_PI / 180) * robot.discrete_time_interval;
    robot.x += delta_x;
    robot.y += delta_y;
